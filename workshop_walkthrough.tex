...
\newcommand{\dragpath}[1]{\textcolor{blue!70!black}{\texttt{\textless drag-and-drop #1\textgreater}}}

\title{LLM Coding Workflow Workshop\\\large}
...
\begin{itemize}[leftmargin=1.2em]
  ...
  \item Produce a short document artifact (Markdown and LaTeX table) generated from local data.
  \item Integrate the plots and tables into a LaTeX conference paper.
\end{itemize}
...
\begin{promptbox}
  \textcolor{blue!70!black}{\texttt{\textless select-and-paste lines 16--24 of src/starter\_analysis.py \textgreater}} Implement this function.
\end{promptbox}
\vspace{1em}

Repeat for the second TODO for \texttt{find\_top\_improvers}.
...
\begin{promptbox}
% \begin{itemize}[leftmargin=1.2em]
  Read failing tests and patch only src/text\_utils.py. Do not change tests unless absolutely necessary. Keep functions simple and robust for edge cases (extra spaces, mixed case, punctuation).
% \end{itemize}
\end{promptbox}
...
\begin{promptbox}
% \begin{enumerate}[leftmargin=1.4em]
  You are helping with a local Python project. Inspect files. Run report generation. Update the report with a short ``failure modes'' section. Do not add new dependencies. Show me exactly what you changed.
% \end{enumerate}
\end{promptbox}
...
\begin{promptbox}
\begin{itemize}[leftmargin=1.2em]
  \item ``\dragpath{src/plot\_metrics.py} Add one additional plot:
  token usage distribution by model. Save it as
  reports/tokens\_by\_model.png. Keep existing plots unchanged,
...
\begin{promptbox}
\begin{itemize}[leftmargin=1.2em]
  \item ``I attached \dragpath{data/students\_scores.csv}. Create
  src/build\_stats\_table.py that generates reports/stats\_table.tex as well as an equivalent markdown table.
...
\begin{itemize}[leftmargin=1.2em]
  ...
  \dragpath{../reports/score\_by\_group.png},
  \dragpath{../reports/tokens\_by\_model.png},
  \dragpath{../reports/accuracy\_vs\_latency.png},
  and the table from \dragpath{../reports/stats\_table.tex}.
